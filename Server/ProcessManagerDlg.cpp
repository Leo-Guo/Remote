...
BOOL CProcessManagerDlg::OnInitDialog()
{
	...
	v1.Format("%s - 进程管理", IsOk != INVALID_SOCKET ? inet_ntoa(ClientAddress.sin_addr) : "本地");
...
BOOL CProcessManagerDlg::ShowClientProcessList()
{
	...
	for (i = 0; Offset < m_ContextObject->m_InDeCompressedBufferData.GetArrayLength() - 1; i++)
	{
		...
		// ItemData 为进程ID
		m_CListCtrl_Client_Process.SetItemData(i, *ProcessID);   //进程ID 设置Hide
		Offset += sizeof(DWORD) + lstrlen(ImageNameData) + lstrlen(ProcessFullPathData) + 2;   //跳过这个数据结构 进入下一个循环
	}
